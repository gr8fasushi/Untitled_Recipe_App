...
export const generateRecipe = onCall(
  ...
  async (request) => {
    ...
    try {
      const response = await groq.chat.completions.create({
        ...
        messages: [
          ...
          {
            role: 'user',
            content: buildRecipePrompt({ ...input, count }),
          },
        ],
    ...
    if (!validated.success) {
      logger.error('generateRecipe: schema validation failed', { error: validated.error.message });
      throw new HttpsError('internal', 'AI response did not match expected schema');
    }

    const recipes = validated.data.recipes.map((r) => ({
      ...r,
    ...
    logger.info('generateRecipe: success', { count: recipes.length });
    return { recipes };
  }
);
