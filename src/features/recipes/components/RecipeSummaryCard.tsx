...
export function RecipeSummaryCard({
  ...
  return (
    <View testID={testID} className="mb-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      ...
      <Text className="text-lg font-nunito-bold text-gray-900 mb-1">{recipe.title}</Text>
      <Text className="text-sm font-nunito text-gray-500 mb-3" numberOfLines={2}>
        {recipe.description}
      </Text>

      <View className="flex-row flex-wrap gap-2 mb-3">
        <View className="rounded-md bg-gray-100 px-2.5 py-1">
          <Text className="text-xs font-nunito text-gray-600">
            ⏱ {recipe.prepTime + recipe.cookTime} min
          </Text>
        </View>
        <View className="rounded-md bg-gray-100 px-2.5 py-1">
          <Text className="text-xs font-nunito text-gray-600">👥 {recipe.servings} servings</Text>
        </View>
        <View className="rounded-md bg-gray-100 px-2.5 py-1">
          <Text className="text-xs font-nunito text-gray-600 capitalize">{recipe.difficulty}</Text>
        </View>
      </View>
